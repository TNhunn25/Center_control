#line 1 "D:\\Power_Central_v4\\Hardware_esp\\protocol_handler.h"
#ifndef PROTOCOL_HANDLER_H
#define PROTOCOL_HANDLER_H

#include <Arduino.h>
#include <ArduinoJson.h>
#include "config.h"
#include "md5.h"
#include "pcf8575_io.h"

#ifndef IO_COMMAND
#define IO_COMMAND 0x02
#endif

// External state owned by the main sketch.
extern const String PRIVATE_KEY;
extern bool outState[4];
extern bool nutVuaNhan[4];
extern PCF8575IO pcf;
extern ManualPcf8575IO manualPcf;
// extern const uint8_t IN_PINS[4];


inline String ResponseJson(int resp_opcode, uint32_t unix_time, int status)
{
    StaticJsonDocument<520> Json;
    Json["opcode"] = resp_opcode;

    JsonObject data = Json.createNestedObject("data");
    data["status"] = status;
    data["out1"] = outState[0] ? 0 : 1;
    data["out2"] = outState[1] ? 0 : 1;
    data["out3"] = outState[2] ? 0 : 1;
    data["out4"] = outState[3] ? 0 : 1;

    Json["time"] = unix_time;

    String data_json;
    serializeJson(Json["data"], data_json);

    String combined = String(resp_opcode) + data_json + String(unix_time) + PRIVATE_KEY;
    Json["auth"] = calculateMD5(combined);

    String out;
    serializeJson(Json, out);
    return out;
}

// Build a JSON command with auth for forwarding to nodes.
inline String CommandJson(const IoCommand &cmd)
{
    StaticJsonDocument<256> dataDoc;
    switch (cmd.opcode)
    {
    case IO_COMMAND:
    {
        dataDoc["out1"] = cmd.out1 ? 0 : 1;
        dataDoc["out2"] = cmd.out2 ? 0 : 1;
        dataDoc["out3"] = cmd.out3 ? 0 : 1;
        dataDoc["out4"] = cmd.out4 ? 0 : 1;
    }
    break;
    default:
        return "";
    }

    String data_json;
    serializeJson(dataDoc, data_json);

    String combined = String(cmd.opcode) + data_json + String(cmd.unix) + PRIVATE_KEY;
    String auth = calculateMD5(combined);

    StaticJsonDocument<512> doc;
    doc["opcode"] = cmd.opcode;
    doc["data"] = dataDoc;
    doc["time"] = cmd.unix;
    doc["auth"] = auth;

    String out;
    serializeJson(doc, out);
    return out;
}

inline String Goi_trangthai(int resp_opcode, uint32_t unix_time)
{
    StaticJsonDocument<256> Json;
    Json["opcode"] = resp_opcode;

    JsonObject data = Json.createNestedObject("data");
    data["out1"] = outState[0] ? 0 : 1;
    data["out2"] = outState[1] ? 0 : 1;
    data["out3"] = outState[2] ? 0 : 1;
    data["out4"] = outState[3] ? 0 : 1;

    for (int i = 0; i < 4; i++)
    {
        bool active = manualPcf.isButtonPressed(i);
        int value = nutVuaNhan[i] ? 2 : (active ? 1 : 0);
        data[String("in") + String(i + 1)] = value;
    }

    Json["time"] = unix_time;

    String data_json;
    serializeJson(Json["data"], data_json);

    String combined = String(resp_opcode) + data_json + String(unix_time) + PRIVATE_KEY;
    Json["auth"] = calculateMD5(combined);

    String out;
    serializeJson(Json, out);
    return out;
}

#endif
