#line 1 "D:\\Power_Central_v4\\led_status.cpp"
#include "led_status.h"

LedStatus::LedStatus()
    : _pin(255),
      _activeHigh(true),
      _state(STATE_NORMAL),
      _lastState((State)255),
      _ledOn(false),
      _lastToggleMs(0),
      _onTimeMs(500),
      _offTimeMs(500)
{
}

void LedStatus::begin(uint8_t pin, bool activeHigh)
{
    _pin = pin;
    _activeHigh = activeHigh;

    pinMode(_pin, OUTPUT);
    writeLed(false);

    _lastState = (State)255;
    _lastToggleMs = millis();
}

void LedStatus::setState(State s)
{
    _state = s;
}

LedStatus::State LedStatus::getState() const
{
    return _state;
}

void LedStatus::update()
{
    if (_pin == 255)
        return;

    const uint32_t now = millis();
    State newState;

    if (has_mode_config && !has_mode_config_on)
    {
        newState = STATE_CONFIG_HOLD;
    }
    else if (!has_mode_config && has_mode_config_on)
    {
        newState = STATE_CONFIG_ACTIVE;
    }
    else if (!has_connect_link && !has_wifi_connection)
    {
        newState = STATE_NO_LINK;
    }
    else if (!has_connect_link && has_wifi_connection)
    {
        newState = STATE_WIFI_CONNECTED;
    }
    else if (!has_mqtt_connection)
    {
        newState = STATE_NO_MQTT;
    }
    else
    {
        newState = STATE_ACTIVE_DATA_ALL;
    }

    if (newState != _state)
    {
        _state = newState;
        _lastState = (State)255;
    }

    if (_state != _lastState)
    {
        _lastState = _state;
        Serial.print(F("[LED] State changed to "));
        Serial.println((int)_state);
        if (_state == STATE_OFF)
        {
            _ledOn = false;
            writeLed(false);
            return;
        }

        getBlinkTiming(_state, _onTimeMs, _offTimeMs);
        _ledOn = true;
        writeLed(true);
        _lastToggleMs = now;
        return;
    }

    if (_state == STATE_OFF)
    {
        _ledOn = false;
        writeLed(false);
        return;
    }

    const uint32_t interval = _ledOn ? _onTimeMs : _offTimeMs;
    if ((uint32_t)(now - _lastToggleMs) < interval)
        return;

    _ledOn = !_ledOn;
    writeLed(_ledOn);
    _lastToggleMs = now;
}

void LedStatus::writeLed(bool on)
{
    if (_activeHigh)
        digitalWrite(_pin, on ? HIGH : LOW);
    else
        digitalWrite(_pin, on ? LOW : HIGH);
}

void LedStatus::getBlinkTiming(State state, uint32_t &onTimeMs, uint32_t &offTimeMs)
{
    switch (state)
    {
    case STATE_NORMAL:
        onTimeMs = 500UL;
        offTimeMs = 500UL;
        break;
    case STATE_NO_MQTT:
        onTimeMs = 500UL;
        offTimeMs = 500UL;
        break;
    case STATE_NO_LINK:
        onTimeMs = 200UL;
        offTimeMs = 200UL;
        break;
    case STATE_WIFI_CONNECTED:
        onTimeMs = 120UL;
        offTimeMs = 380UL;
        break;
    case STATE_ACTIVE_DATA_ALL:
        onTimeMs = 50UL;
        offTimeMs = 950UL;
        break;
    case STATE_CONFIG_HOLD:
        onTimeMs = 100UL;
        offTimeMs = 100UL;
        break;
    case STATE_CONFIG_ACTIVE:
        onTimeMs = 950UL;
        offTimeMs = 50UL;
        break;
    case STATE_OFF:
        onTimeMs = 0;
        offTimeMs = 0;
        break;
    }
}
